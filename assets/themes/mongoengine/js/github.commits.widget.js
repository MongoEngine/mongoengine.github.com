            ...
            gh.commit.forBranch(user, repo, branch, function (data) {
                ...
                for (var c = 0; c < totalCommits; c++) {
                    row.append(
                        ...
                        avatar(commits[c].author, avatarSize) + '</div></div>' +
                        '<div class="commit-text"><p>' + author(commits[c].author.login) +
                ...
                function avatar(author, size) {
                    var emailHash = hex_md5(author.email);
                    return '<a target="_blank" href="https://github.com/' + author.login + '"><img class="github-avatar" src="http://www.gravatar.com/avatar/' + emailHash + '?s=' + size + '"/></a>';
                }
